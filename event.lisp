...
(define-event-handler :exposure (window count)
  (if (zerop count)
	  ...
	  (update-title (decorate (find-window window)))))
  t)
  ...
  (labels ((process-mapped-window (xmaster)
	   ...
	   (process-withdrawn-window (xroot)
	     (let* ((screen (find-screen xroot))
		    (id (loop for id being the hash-keys in (withdrawn-windows screen) using (hash-value xwindow)
				when (not (probe-xwindow xwindow))
				return id)))
	       (remhash id (withdrawn-windows *display*))
	       (remhash id (withdrawn-windows screen)))))
