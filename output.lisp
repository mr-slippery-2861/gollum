  ...
  (labels ((unpack (command index)
    ...
    (multiple-value-bind (raw-string commands) (parse-color-controling content)
      (let* ((gc (output-gc screen))
	     ...
	     (reverse-p nil))
	(setf reverse-p (do-command gc (cdar commands)))
	(when (eql reverse-p :global)
	  (xlib:draw-rectangle window gc 0 (+ offset-y *internal-window-vertical-padding*) (xlib:drawable-width window) (+ (xlib:font-ascent font) (xlib:font-descent font)) t)
	  (setf reverse-p nil
		commands (cdr commands)
		end (if commands (caar commands) (length raw-string))
		total (1- total)
		(xlib:gcontext-foreground gc) (alloc-color *background-color* screen)
		(xlib:gcontext-background gc) (alloc-color *foreground-color* screen)))
...
(defmethod output-to-window ((screen screen) xwindow gcontext gravity content)
  ...
  (let* ((output-font (output-font screen))
    ...
    (setup-window-for-drawing-glyphs screen xwindow gravity gcontext output-font (mapcar #'parse-color-controling lines))
